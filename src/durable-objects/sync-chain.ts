...
const MAX_BATCHES_PER_RUN = 200;   // hard safety cap; 200 × 20 = 4000 PRs covered
...
export class SyncChain extends DurableObject<Env> {
  ...
  private async handleStart(): Promise<Response> {
    ...
    if (status === "running") {
      ...
      return Response.json({ ok: true, alreadyRunning: true, page, batches });
    }
  ...
  private async handleStatus(): Promise<Response> {
    ...
    return Response.json({ status, page, batches, processed, startedAt, finishedAt, lastError });
  }
  ...
  async alarm(): Promise<void> {
    ...
    if (batches >= MAX_BATCHES_PER_RUN) {
      await this.ctx.storage.put({
        status: "stopped",
        finishedAt: Date.now(),
        lastError: `safety cap: ${batches} batches`,
      });
      await syncLog(this.env, "warn", "sync.chain.stopped",
        `safety cap hit at ${batches} batches, stopping`, { batches, page });
      return;
    }
