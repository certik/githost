...
export async function fullResync(
  ...
  switch (resource) {
    case "prs":     path = `${base}/pulls?state=all&sort=updated&direction=desc&per_page=50`; break;
    case "issues":  path = `${base}/issues?state=all&sort=updated&direction=desc&per_page=50${since ? `&since=${encodeURIComponent(since)}` : ""}`; break;
    case "comments":path = `${base}/issues/comments?sort=updated&direction=desc&per_page=100${since ? `&since=${encodeURIComponent(since)}` : ""}`; break;
  }

  let newestUpdatedAt: string | undefined;
  let safety = 10; // page cap per sweep — cron runs nightly, backfill happens over a few nights

  while (path && safety-- > 0) {
    const res = await gh(env, { installationId, path });
